...
const AdminHomePage = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      // 병렬로 여러 API 호출
      const [usersData, stadiumsData, matchesData, todayResData] =
        await Promise.allSettled([
          ADMIN.getAllUsers(),
          ADMIN.getAllStadiums(),
          ADMIN.getAllMatches(),
          ADMIN.getTodayReservations(),
        ]);
      // 각 API 응답 처리
      let totalUsers = 0;
      ...
      if (todayResData && todayResData.status === "fulfilled") {
        const res = todayResData.value;
        if (Array.isArray(res)) {
          todayReservations = res.length;
        } else {
          todayReservations = res.count ?? res.data?.count ?? 0;
        }
        console.log(
          "오늘 예약 수 응답:",
          res,
          "최종 todayReservations:",
          todayReservations
        );
      }
      const newStats = {
