...
function SignUp () {
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    if (email.trim() && password.trim() && passwordConfirm.trim() && name.trim() && birth && gender) {
      if (password !== passwordConfirm) {
        alert("비밀번호가 일치하지 않습니다.");
        return;
      }
      const bodyData = { email: email, password: password, name: name, birth: birth, gender: gender };
      console.log('회원가입 요청 body:', bodyData); // body에 전달되는 내용 출력
      fetch("http://checksabca.tplinkdns.com:44445/api/user/signup", {
        method: 'POST',
        headers: {
    ...
    <div className="container mx-auto mt-10 bg-white border border-gray-300 rounded-lg shadow-lg p-6 max-w-md">
      ...
      <form onSubmit={submitHandler}>
        ...
        <div className="flex justify-center mt-6">
          <button type='submit'
          className='mt-6 bg-sky-500 hover:bg-sky-700 text-white py-2 px-4 rounded-md shadow w-40'>가입하기</button>
        </div>
      </form>
    </div>
  </div>
